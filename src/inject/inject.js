$(document).ready(function() {
	if ($('.toc-diff-stats').length) {
		...
		$toc.find('.button-group .minibutton:eq(1)').attr('style',
			'border-top-right-radius: 3px;border-bottom-right-radius: 3px;');
		$modal.find('.minibutton').attr('style',
			'border-top-right-radius: 3px;border-bottom-right-radius: 3px;');

		$toc.find('.button-group').append($modal);
		...
		var hide_filtered_diffs = function() {
			...
			$.each(extensions, function(k, ext) {
				$files = $files_arr.filter(function() {
					var path = $(this).find('.file-header')
					return path.attr('data-path').match(k + '$')
				});

				$links = $toc.find('a[data-extension="' + k + '"]');
				if (ext["hidden"]) {
					var $hidden_by_filter =
						'<i class="filter-notice">(hidden by filter)</i>';
					$files
			...
			$('.filtered-string').text(Object.keys(extensions).length -
				hidden_extension_length);

			if (hidden_length > 0) {
				$toc.find('.explain>strong').eq(0).text((files_length - hidden_length) +
					' changed files (' + hidden_length + ' hidden by filter)')
